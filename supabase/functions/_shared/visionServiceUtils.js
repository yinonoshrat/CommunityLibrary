...
4. For each book return its appropriate genre from the following list in Hebrew:
...
export function groupTextBlocks(blocks) {
  ...
  // Sort by Y position
  const sorted = [...blocks].sort((a, b) => a.position.centerY - b.position.centerY);
...
/**
 * Parse Google Cloud Vision API response into structured blocks
 * @param {Array} textAnnotations - The textAnnotations array from Vision API
 * @returns {Object} - { fullText, blocks }
 */
export function parseVisionResponse(textAnnotations) {
  ...
  const blocks = textAnnotations.slice(1).map((annotation, index) => {
